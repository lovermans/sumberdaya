...
<head> <noscript> <meta HTTP-EQUIV="refresh" content="0;url='{{ route('perlu-javascript') }}'"> </noscript>
    <meta charset="utf-8">
...
<body data-tematerang=""> <a class="skip-navigation tcetak" href="#main" tabindex="0">Langsung Ke Konten Utama</a><input
    ...
    <header class="tcetak">
        <section><label for="nav" id="tbl-nav" title="Menu"><svg class="on" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <h1>SDM</h1><label for="tema" id="tbl-tema" onclick="" title="Ubah Tema"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <use xlink:href="{{ asset(mix('/ikon.svg')) . '#tema' }}"
                        xmlns:xlink="http://www.w3.org/1999/xlink"></use>
                </svg></label><label for="menu" id="tbl-menu" onclick="" title="Akun"><img id="akun" @if(optional(auth()->user())->tautan_foto == null) class="svg" @endif
                    src="{{ auth()->user()->tautan_foto ?? asset(mix('/ikon.svg')) . '#akun' }}" alt="{{ auth()->user()->nama ?? 'foto akun' }}" alt="{{ auth()->user()->nama ?? 'foto akun' }}" loading="lazy"></label>
            <div class="bersih"></div>
        </section>
    </header>
    @auth <aside class="tcetak">
    ...
    <nav class="tcetak">
        <div class="menu-t {{ request()->routeIs('mulai') ? 'aktif' : '' }}"><a class="nav-xhr" href="{{ route('mulai') }}"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <use xlink:href="{{ asset(mix('/ikon.svg')) . '#aplikasi' }}"
                xmlns:xlink="http://www.w3.org/1999/xlink"></use>
        </svg>Pilih Sumber Daya</a></div>
        <div class="menu-j"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <use xlink:href="{{ asset(mix('/ikon.svg')) . '#personil' }}"
                    xmlns:xlink="http://www.w3.org/1999/xlink"></use>
            </svg>Data SDM</div>
        <ul class="submenu">
            <li><a class="nav-xhr" href="{{ route('login') }}">Masuk</a></li>
            <li><a class="nav-xhr" href="{{ url('/sdm') }}">Data SDM</a></li>
            <li><a class="nav-xhr" href="{{ url('/asdasadad') }}">Sanksi SDM</a></li>
        </ul>
        <div class="menu-t"><a class="nav-xhr" href="{{ url('/testing') }}"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <use xlink:href="{{ asset(mix('/ikon.svg')) . '#riwayat' }}"
                        xmlns:xlink="http://www.w3.org/1999/xlink"></use>
                </svg>Riwayat Status SDM</a></div>
        <div class="menu-j"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <use xlink:href="{{ asset(mix('/ikon.svg')) . '#riwayat' }}"
                    xmlns:xlink="http://www.w3.org/1999/xlink"></use>
            </svg>Pengaturan</div>
        <ul class="submenu">
            <li><a class="nav-xhr" href="">Entitas</a></li>
        </ul>
