...
@section('isi')
    <div id="penempatan_sdm_hapus">
        <form class="form-xhr kartu" id="form_penempatanSDMHapus" method="POST" action="{{ $app->url->current() }}">
            ...
            <div class="isian gspan-4">
                <p>
                    Yakin menghapus data penempatan : {{ $penem->sdm_no_absen }} - {{ $penem->sdm_nama }} -
                    {{ strtoupper($app->date->make($penem->penempatan_mulai)?->translatedFormat('d F Y')) }} -
                    {{ $penem->penempatan_lokasi }} - {{ $penem->penempatan_kontrak }} - {{ $penem->penempatan_ke }} ?
                </p>
