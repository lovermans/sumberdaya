        ...
        <div class="kartu form">
            @isset($penem)
                ...
                <div class="isian gspan-4">
                    ...
                    @if ($app->filesystem->exists($berkasPenempatan = 'sdm/penempatan/berkas/' . $penem->sdm_no_absen . ' - ' . $penem->penempatan_mulai . '.pdf'))
                        <iframe class="berkas tcetak"
                            src="{{ $app->url->route('sdm.berkas', ['berkas' => $berkasPenempatan . '?id=' . filemtime(storage_path('app/' . $berkasPenempatan))]) }}"
                        ...
                        <a class="sekunder tcetak"
                            href="{{ $app->url->route('sdm.berkas', ['berkas' => $berkasPenempatan . '?id=' . filemtime(storage_path('app/' . $berkasPenempatan))]) }}"
                ...
                @if (str()->contains($app->request->user()->sdm_hak_akses, 'SDM-PENGURUS'))
                    <div class="isian" id="penempatan-cetakFormulir">
                        <select class="pil-saja tombol"
                            ...
                            <option value="{{ $app->url->route('sdm.penempatan.formulir-penilaian-sdm', ['uuid' => $penem->penempatan_uuid]) }}">
                                PENILAIAN KINERJA
                            </option>

                            <option value="{{ $app->url->route('sdm.penempatan.formulir-perubahan-status-sdm', ['uuid' => $penem->penempatan_uuid]) }}">
                                PERUBAHAN STATUS
                            </option>
                        </select>
                    </div>
