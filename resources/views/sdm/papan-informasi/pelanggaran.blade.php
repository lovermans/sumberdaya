@isset($pelanggarans)
    <details class="kartu">
        ...
        <div class="scroll-margin" id="tabel_pelanggaranSDM_sematan"></div>

        <div class="kartu" id="tabel_pelanggaranSDM">
                ...
                <table class="tabel">
                    ...
                    <tbody>
                        @forelse ($pelanggarans as $no => $pelanggaran)
                            <tr @class([
                                'biru' => str()->contains($pelanggaran->langgar_tkontrak, 'OS-'),
                            ])>
                                <th>
                                    <div class="pil-aksi">
                                ...
                                <td class="profil">
                                    <div @class(['merah' => $pelanggaran->langgar_tsdm_tgl_berhenti])>
                                        ...
                                        <a class="isi-xhr taut-akun" href="{{ $app->url->route('sdm.akun', ['uuid' => $pelanggaran->langgar_tsdm_uuid]) }}">
                                            <img src="{{ $app->filesystem->exists('sdm/foto-profil/' . $pelanggaran->langgar_no_absen . '.webp')
                                                ? $app->url->route('sdm.tautan-foto-profil', [
                                                    'berkas_foto_profil' =>
                                                        $pelanggaran->langgar_no_absen . '.webp' . '?id=' . filemtime($app->storagePath('app/sdm/foto-profil/' . $pelanggaran->langgar_no_absen . '.webp')),
                                                ])
                                                : $app->url->asset($app->make('Illuminate\Foundation\Mix')('/images/blank.webp')) }}"
                                                title="{{ $pelanggaran->langgar_tsdm_nama ?? 'foto akun' }}"
                                                alt="{{ $pelanggaran->langgar_tsdm_nama ?? 'foto akun' }}" @class([
                                                    'akun',
                                    ...
                                    <div @class(['merah' => $pelanggaran->langgar_psdm_tgl_berhenti])>
                                        ...
                                        <a class="isi-xhr taut-akun" href="{{ $app->url->route('sdm.akun', ['uuid' => $pelanggaran->langgar_psdm_uuid]) }}">
                                            <img src="{{ $app->filesystem->exists('sdm/foto-profil/' . $pelanggaran->langgar_pelapor . '.webp')
                                                ? $app->url->route('sdm.tautan-foto-profil', [
                                                    'berkas_foto_profil' =>
                                                        $pelanggaran->langgar_pelapor . '.webp' . '?id=' . filemtime($app->storagePath('app/sdm/foto-profil/' . $pelanggaran->langgar_pelapor . '.webp')),
                                                ])
                                                : $app->url->asset($app->make('Illuminate\Foundation\Mix')('/images/blank.webp')) }}"
                                                title="{{ $pelanggaran->langgar_psdm_nama ?? 'foto akun' }}"
                                                alt="{{ $pelanggaran->langgar_psdm_nama ?? 'foto akun' }}" @class([
                                                    'akun',
                                                    'svg' => !$app->filesystem->exists(
                                ...
                                <td>
                                    ...
                                    <b>Berakhir pada </b> :
                                    {{ strtoupper($app->date->make($pelanggaran->sanksi_selesai_sebelumnya)?->translatedFormat('d F Y')) }}
                                    <br>
                                    ...
                                    <b>Mulai </b> :
                                    {{ strtoupper($app->date->make($pelanggaran->final_sanksi_mulai)?->translatedFormat('d F Y')) }}<br>
                                    <b>Selesai </b> :
                                    {{ strtoupper($app->date->make($pelanggaran->final_sanksi_selesai)?->translatedFormat('d F Y')) }}<br>
