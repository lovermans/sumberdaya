@isset($perminSDMS)
    <details class="kartu">
        ...
        <div class="scroll-margin" id="tabel_perminSDM_sematan"></div>

        <div class="kartu" id="tabel_perminSDM">
                        ...
                        @forelse ($perminSDMS as $no => $perminSDM)
                            <tr>
                                ...
                                <td class="profil">
                                    ...
                                    <a class="isi-xhr taut-akun" href="{{ $app->url->route('sdm.akun', ['uuid' => $perminSDM->sdm_uuid]) }}">
                                        <img src="{{ $app->filesystem->exists('sdm/foto-profil/' . $perminSDM->tambahsdm_sdm_id . '.webp')
                                            ? $app->url->route('sdm.tautan-foto-profil', [
                                                'berkas_foto_profil' =>
                                                    $perminSDM->tambahsdm_sdm_id . '.webp' . '?id=' . filemtime($app->storagePath('app/sdm/foto-profil/' . $perminSDM->tambahsdm_sdm_id . '.webp')),
                                                false,
                                            ])
                                            : $app->url->asset($app->make('Illuminate\Foundation\Mix')('/images/blank.webp')) }}"
                                            title="{{ $perminSDM->sdm_nama ?? 'foto akun' }}" alt="{{ $perminSDM->sdm_nama ?? 'foto akun' }}"
                                            @class([
                                                'akun',
                                    ...
                                    <b>Dibutuhkan</b> :
                                    {{ strtoupper($app->date->make($perminSDM->tambahsdm_tgl_dibutuhkan)?->translatedFormat('d F Y')) }}
                                </td>
                                ...
                                <td>
                                    ...
                                    <b>Jml Terpenuhi</b> :
                                    <u>
                                        <a class="isi-xhr" href="{{ $app->url->route('sdm.penempatan.riwayat', ['kata_kunci' => $perminSDM->tambahsdm_no]) }}">
