...
<div id="permintambahsdm">
    <form id="form_permintambahsdm" class="form-xhr kartu" method="POST" action="{{ $urlRangka->current() }}">
        ...
        <div class="isian" style="place-self:start start;display: inline-block">
            <label for="permintambahsdmJumlah">Jumlah Dibutuhkan</label>
            <input id="permintambahsdmJumlah" type="number" name="tambahsdm_jumlah" value="{{ $rekRangka->old('tambahsdm_jumlah', $permin->tambahsdm_jumlah ?? null) }}" min="0" inputmode="numeric" required style="width:70%">
