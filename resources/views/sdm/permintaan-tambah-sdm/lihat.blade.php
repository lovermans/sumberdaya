        ...
        @isset($permin)
            ...
            <div class="isian gspan-4">
                ...
                @if ($storageRangka->exists('sdm/permintaan-tambah-sdm/'.$permin->tambahsdm_no.'.pdf'))
                    <iframe class="tcetak" src="{{ $urlRangka->route('sdm.permintaan-tambah-sdm.berkas', ['berkas' => $permin->tambahsdm_no . '.pdf' . '?' . filemtime($appRangka->storagePath('app/sdm/permintaan-tambah-sdm/' . $permin->tambahsdm_no . '.pdf'))], false) }}" title="Berkas Permintaan SDM" loading="lazy" style="width:100%;height:auto;aspect-ratio:4/3"></iframe>
                @else
                    <p class="merah">Tidak ada berkas terunggah.</p>
                @endif
                <a class="isi-xhr sekunder tcetak" data-rekam="false" data-laju="true" data-tujuan="#permintaan-sdm_sematan_lihat" href="{{ $urlRangka->route('sdm.permintaan-tambah-sdm.formulir', ['uuid' => $permin->tambahsdm_uuid], false) }}">
