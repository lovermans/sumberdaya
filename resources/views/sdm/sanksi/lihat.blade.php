    ...
    <div class="kartu form">
        ...
        <div class="isian gspan-4">
            ...
            <iframe class="berkas tcetak"
                src="{{ $urlRangka->route('sdm.pelanggaran.berkas', ['berkas' => $sanksi->sanksi_lap_no . '.pdf' . '?' . filemtime(storage_path('app/sdm/pelanggaran/berkas/' . $sanksi->sanksi_lap_no . '.pdf'))], false) }}"
            ...
            <a class="sekunder tcetak" target="_blank" title="Unduh Berkas Terunggah"
                href="{{ $urlRangka->route('sdm.pelanggaran.berkas', ['berkas' => $sanksi->sanksi_lap_no . '.pdf' . '?' . filemtime(storage_path('app/sdm/pelanggaran/berkas/' . $sanksi->sanksi_lap_no . '.pdf'))], false) }}">
        ...
        <div class="isian gspan-4">
            ...
            <iframe class="berkas tcetak"
                src="{{ $urlRangka->route('sdm.sanksi.berkas', ['berkas' => $sanksi->sanksi_no_absen . ' - ' . $sanksi->sanksi_jenis . ' - ' . $sanksi->sanksi_mulai . '.pdf' . '?' . filemtime(storage_path('app/sdm/sanksi/berkas/' . $sanksi->sanksi_no_absen . ' - ' . $sanksi->sanksi_jenis . ' - ' . $sanksi->sanksi_mulai . '.pdf'))], false) }}"
                title="Dokumen Sanksi SDM" loading="lazy"
                onload="if (this.contentDocument.body.id == 'badan-dokumen') this.remove()"></iframe>

            <a class="sekunder tcetak" target="_blank" title="Unduh Berkas Terunggah"
                href="{{ $urlRangka->route('sdm.sanksi.berkas', ['berkas' => $sanksi->sanksi_no_absen . ' - ' . $sanksi->sanksi_jenis . ' - ' . $sanksi->sanksi_mulai . '.pdf' . '?' . filemtime(storage_path('app/sdm/sanksi/berkas/' . $sanksi->sanksi_no_absen . ' - ' . $sanksi->sanksi_jenis . ' - ' . $sanksi->sanksi_mulai . '.pdf'))], false) }}">
                <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <use xlink:href="{{ $mixRangka('/ikon.svg') . '#unduh' }}"
                        xmlns:xlink="http://www.w3.org/1999/xlink"></use>
                </svg>
