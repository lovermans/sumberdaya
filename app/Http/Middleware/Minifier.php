...
abstract class Minifier
{
    ...
    {
        if ($this->isEnable() && (strtolower(strtok($response->headers->get('Content-Type'), ';')) === 'text/html') && (is_object($response) && $response instanceof Response)) {
            return true;
        }
    ...
    protected function loadDom(string $html, bool $force = false)
    {
        ...
        @static::$dom->loadHTML($html, LIBXML_HTML_NODEFDTD | LIBXML_HTML_NOIMPLIED | LIBXML_SCHEMA_CREATE);
    }
