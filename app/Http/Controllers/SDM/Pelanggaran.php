...
class Pelanggaran
{
    public function index(Rule $rule, FungsiStatis $fungsiStatis)
    {
        ...
        if ($validator->fails()) {
            return $app->redirect->route('sdm.pelanggaran.data')->withErrors($validator)->withInput();
        };
        ...
        if ($reqs->unduh == 'excel') {
            ...
            $filename = 'eksporpelanggaransdm-' . date('YmdHis') . '.xlsx';
            ...
            $cari->clone()->chunk(100, function ($hasil) use (&$x, $worksheet) {
                if ($x == 1
                    ...
                    $list = $hasil->map(function ($x) {
                        return collect($x)->except(['langgar_uuid']);
                    })->toArray();
                ...
                $x += count($hasil);
                echo '<p>Status : Memproses ' . ($x - 2) . ' data laporan pelanggaran SDM.</p>';
            });
    ...
    public function tambah(FungsiStatis $fungsiStatis)
    {
        ...
        if ($reqs->isMethod('post')) {
            ...
            $dataMap = array_map(function ($x, $y) use ($reqs) {
                return ['langgar_no_absen' => $x] + ['langgar_lap_no' => date('Y') . date('m') . str($y)->padLeft(4, '0')]
            ...
            if ($berkas) {
                ...
                array_walk($valid, function ($x, $y) use ($berkas) {
                    $berkas->storeAs('sdm/pelanggaran', $x . '.pdf');
                });
            }
